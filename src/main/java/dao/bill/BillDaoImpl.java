...
public class BillDaoImpl implements BillDao {
    ...
    public int addBill(Connection conn, Bill bill) {
        ...
        if(conn != null) {
            try {
                String sql = "insert into smbms_bill (isPayment, createdBy,creationDate,providerId)" +
                        " values(?,?,?,?)";
                Object[] params = {bill.getIsPayment(),bill.getCreatedBy(), bill.getCreationDate(),bill.getProviderId()};
                for (int i = 0; i < params.length; i++){
                    System.out.println( "param: "+params[i].toString());
                }
                affectedRows = DB.executeUpdate(conn,sql,params);
                System.out.println("affectRow ---->"+affectedRows);
            }
    ...
    public Bill findById(Connection conn, Integer id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                bill.setTotalPrice(rs.getBigDecimal("proPhone"));
    ...
    public List<Bill> getBillList(Connection conn, String productName, int proId,int currPage, int pageSize,int isPayment) {
        ...
        if(conn != null) {
            StringBuffer sql = new StringBuffer();
            sql.append("select * from smbms_bill b, smbms_provider p,smbms_user u where b.providerId = p.id AND b.createdBy = u.id ");
            if(productName != null || !productName.equals("")) {
